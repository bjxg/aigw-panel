...
审计日期：2026-04-13  
审计范围：`aigw` Go 网关核心、`aigw-panel` React 管理端、两个子项目的测试与构建配置。  
...
- 入口抽样：`aigw/internal/api/server.go`、管理接口、OAuth/认证文件路径、`aigw-panel/src/app/AppRouter.tsx`、认证状态和大页面。
- 基线验证：`aigw` 执行 `go test ./...` 通过；`aigw-panel` 执行 `bun run check` 构建通过，但 lint 输出 33 个 warning，Vite 输出多个大 chunk warning。
- 历史参考：`aigw/docs/internal-review/code-quality-review.md` 已有后端质量审查，本报告只把它作为历史背景，优先采用本次扫描证据。
...
| 子系统 | 当前状态 | 结论 |
| --- | --- | --- |
| `aigw` | `go test ./...` 通过 | 后端测试基线可用，但仍有上下文、请求体、协程生命周期等结构性风险 |
...
- 根目录实际包含两个独立子仓库：`aigw` 与 `aigw-panel`。
- 原本只有 `aigw/docs` 和 `aigw-panel/docs`，没有根级系统治理文档区。
- `aigw-panel` README 明确是 `aigw` 的管理端，但文档、CI、规范仍按两个项目分散沉淀。
...
| 文件 | 行数 | 主要问题 |
| --- | ---: | --- |
| `aigw/sdk/cliproxy/auth/conductor.go` | 2547 | 认证调度、执行、刷新、持久化、选择器相关逻辑高度集中 |
| `aigw/internal/api/handlers/management/auth_files.go` | 2494 | 管理端认证文件、OAuth 回调、异步流程、状态管理混合 |
| `aigw/internal/config/config.go` | 1904 | 配置结构过大，领域边界不清晰 |
| `aigw/internal/api/handlers/management/config_lists.go` | 1714 | 配置列表相关逻辑集中 |
| `aigw/internal/runtime/executor/antigravity_executor.go` | 1690 | 单 Provider executor 承担过多协议细节 |
| `aigw/internal/api/server.go` | 1550 | Gin engine、路由、管理面板、动态配置、服务状态混合 |
...
- 排除测试和 translator 生成/转换目录后，`aigw/internal` 与 `aigw/sdk` 仍扫描到约 103 处 `io.ReadAll`。
- 项目已有安全封装 `aigw/internal/api/bodyutil/bodyutil.go`，其中 `DefaultRequestBodyLimit` 为 16 MiB，并使用 `http.MaxBytesReader` 或 `io.LimitedReader`。
...
- 排除测试和 translator 后，`aigw/internal` 与 `aigw/sdk` 扫描到约 102 处 `context.Background()`。
...
- 排除测试和 translator 后，`aigw/internal` 与 `aigw/sdk` 扫描到约 54 处 `go func()`。
...
- `aigw/internal/api/server.go` 创建主 `http.Server` 时仅设置 `Addr` 和 `Handler`。
...
- 管理接口中，`aigw/internal/api/handlers/management/handler.go` 通过 `c.ClientIP()` 判断 `localClient`，再决定是否允许远程管理。
...
- `aigw/internal/api/server.go` 的 `corsMiddleware()` 对非管理路径设置：
...
- `aigw/internal/api/handlers/management/auth_files.go` 的 raw JSON 上传路径使用 `bodyutil.AuthFileBodyLimit` 限制为 2 MiB。
...
- `aigw/internal/api/handlers/management/auth_files.go` 的 `DownloadAuthFile` 使用 `os.ReadFile(full)` 读完整文件后再 `c.Data(...)` 返回。
...
- `aigw/internal/config/config.go` 中 `DefaultPprofAddr` 为 `127.0.0.1:8316`，默认是本地绑定。
- `aigw/sdk/cliproxy/pprof_server.go` 在启用后直接注册 `/debug/pprof/*` 路由，没有认证层。
