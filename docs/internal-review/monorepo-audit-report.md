...
审计日期：2026-04-13  
审计范围：`aigw-server` Go 网关核心、`aigw-panel` React 管理端、两个子项目的测试与构建配置。  
...
- 入口抽样：`aigw-server/internal/api/server.go`、管理接口、OAuth/认证文件路径、`aigw-panel/src/app/AppRouter.tsx`、认证状态和大页面。
- 基线验证：`aigw-server` 执行 `go test ./...` 通过；`aigw-panel` 执行 `bun run check` 构建通过，但 lint 输出 33 个 warning，Vite 输出多个大 chunk warning。
- 历史参考：`aigw-server/docs/internal-review/code-quality-review.md` 已有后端质量审查，本报告只把它作为历史背景，优先采用本次扫描证据。
...
| 子系统 | 当前状态 | 结论 |
| --- | --- | --- |
| `aigw-server` | `go test ./...` 通过 | 后端测试基线可用，但仍有上下文、请求体、协程生命周期等结构性风险 |
| `aigw-panel` | `bun run check` 通过 | 构建可用，但 lint 有 33 个 warning，说明前端代码卫生已经开始退化 |
| `aigw-panel` bundle | `vendor-echarts` 约 1.14 MB，`vendor-markdown` 约 779 KB，`index` 约 639 KB，`ConfigPage` 约 152 KB，`AuthFilesPage` 约 101 KB | 路由级懒加载已存在，但重依赖和页面级 chunk 仍明显偏大 |
...
最需要优先处理的是前端维护性。`aigw-panel` 已经出现多个 1000 行以上文件，最大页面超过 4000 行；这些文件把 API 调用、状态、派生计算、表格、图表、弹窗、缓存和交互流程放在一个文件里，后续任何功能都会继续放大变更风险。
...
- 根目录实际包含两个独立子仓库：`aigw-server` 与 `aigw-panel`。
- 原本只有 `aigw-server/docs` 和 `aigw-panel/docs`，没有根级系统治理文档区。
- `aigw-panel` README 明确是 `aigw-server` 的管理端，但文档、CI、规范仍按两个项目分散沉淀。
...
| 文件 | 行数 | 主要问题 |
| --- | ---: | --- |
| `aigw-panel/src/modules/auth-files/AuthFilesPage.tsx` | 4095 | 页面、缓存、表格、Quota、OAuth、图表、筛选、多个弹窗混在一起 |
| `aigw-panel/src/modules/providers/ProvidersPage.tsx` | 1974 | 多 Provider 表单、模型发现、状态栏、多个 CRUD 流程混在一个组件 |
| `aigw-panel/src/modules/apikey-lookup/ApiKeyLookupPage.tsx` | 1721 | 公开查询、图表、日志表、统计派生逻辑集中 |
| `aigw-panel/src/utils/usage.ts` | 1664 | 统计、脱敏、图表序列、价格计算等工具过度集中 |
| `aigw-panel/src/modules/api-keys/ApiKeysPage.tsx` | 1517 | API Key 管理、日志、配额、权限和弹窗耦合 |
| `aigw-panel/src/modules/monitor/LogContentModal.tsx` | 1479 | 内容加载、Markdown/XML 渲染、复制导出、状态切换耦合 |
| `aigw-panel/src/modules/config/visual/VisualConfigEditor.tsx` | 1249 | 可视化配置编辑器过大，缺少子域边界 |
| `aigw-panel/src/modules/config/ConfigPage.tsx` | 897 | 配置源编辑、运行时配置、可视化编辑和保存流程耦合 |
...
| 文件 | 行数 | 主要问题 |
| --- | ---: | --- |
| `aigw-server/sdk/cliproxy/auth/conductor.go` | 2547 | 认证调度、执行、刷新、持久化、选择器相关逻辑高度集中 |
| `aigw-server/internal/api/handlers/management/auth_files.go` | 2494 | 管理端认证文件、OAuth 回调、异步流程、状态管理混合 |
| `aigw-server/internal/config/config.go` | 1904 | 配置结构过大，领域边界不清晰 |
| `aigw-server/internal/api/handlers/management/config_lists.go` | 1714 | 配置列表相关逻辑集中 |
| `aigw-server/internal/runtime/executor/antigravity_executor.go` | 1690 | 单 Provider executor 承担过多协议细节 |
| `aigw-server/internal/api/server.go` | 1550 | Gin engine、路由、管理面板、动态配置、服务状态混合 |
...
- 简单 helper 文件存在大量“翻译式注释”，例如 `aigw-panel/src/utils/helpers.ts` 中 `防抖函数`、`节流函数`、`生成唯一 ID`、`延迟函数` 等注释解释的是函数名本身。
- 复杂页面如 `AuthFilesPage.tsx`、`ProvidersPage.tsx` 则缺少模块级结构说明、业务规则说明和边界条件注释。
- `aigw-panel/src/utils/encryption.ts` 只说明“加密工具函数”，但没有说明该实现只是客户端混淆，不应作为强安全边界。
...
- `aigw-panel/src/modules` 下当前仅扫描到 10 个单测文件，`aigw-panel/e2e` 下 3 个 E2E 文件。
...
- 当前同时存在两套 API 层：
  `aigw-panel/src/lib/http/apis` 15 个文件，
  `aigw-panel/src/services/api` 16 个文件。
...
- `AuthFileItem` 同时定义于 `aigw-panel/src/lib/http/types.ts` 和 `aigw-panel/src/types/authFile.ts`。
- `OAuthProvider` 同时定义于 `aigw-panel/src/lib/http/types.ts` 和 `aigw-panel/src/types/oauth.ts`。
...
- `aigw-panel/src/modules/auth/AuthProvider.tsx` 在 `readAuthSnapshot` 中读取 `localStorage.getItem(AUTH_STORAGE_KEY)`。
...
- `aigw-panel/src/services/storage/secureStorage.ts` 默认把数据写入 `localStorage`。
- `aigw-panel/src/utils/encryption.ts` 使用固定 `SECRET_SALT`、host、userAgent 生成 key，并通过 XOR 做变换。
...
- 排除测试和 translator 生成/转换目录后，`aigw-server/internal` 与 `aigw-server/sdk` 仍扫描到约 103 处 `io.ReadAll`。
- 项目已有安全封装 `aigw-server/internal/api/bodyutil/bodyutil.go`，其中 `DefaultRequestBodyLimit` 为 16 MiB，并使用 `http.MaxBytesReader` 或 `io.LimitedReader`。
...
- 排除测试和 translator 后，`aigw-server/internal` 与 `aigw-server/sdk` 扫描到约 102 处 `context.Background()`。
...
- 排除测试和 translator 后，`aigw-server/internal` 与 `aigw-server/sdk` 扫描到约 54 处 `go func()`。
...
- `aigw-server/internal/api/server.go` 创建主 `http.Server` 时仅设置 `Addr` 和 `Handler`。
...
- 管理接口中，`aigw-server/internal/api/handlers/management/handler.go` 通过 `c.ClientIP()` 判断 `localClient`，再决定是否允许远程管理。
...
- `aigw-server/internal/api/server.go` 的 `corsMiddleware()` 对非管理路径设置：
...
- `aigw-server/internal/api/handlers/management/auth_files.go` 的 raw JSON 上传路径使用 `bodyutil.AuthFileBodyLimit` 限制为 2 MiB。
...
- `aigw-server/internal/api/handlers/management/auth_files.go` 的 `DownloadAuthFile` 使用 `os.ReadFile(full)` 读完整文件后再 `c.Data(...)` 返回。
- 前端 `aigw-panel/src/lib/http/apis/auth-files.ts` 用 `getText()` 拉取完整文本。
...
- `aigw-server/internal/config/config.go` 中 `DefaultPprofAddr` 为 `127.0.0.1:8316`，默认是本地绑定。
- `aigw-server/sdk/cliproxy/pprof_server.go` 在启用后直接注册 `/debug/pprof/*` 路由，没有认证层。
...
- `aigw-panel/src/modules/apikey-lookup/ApiKeyLookupPage.tsx` 在提交查询时把真实 key 写入 URL：`url.searchParams.set("api_key", val)`。
...
- `aigw-panel/src/app/AppRouter.tsx` 已使用 `lazy` 做路由级拆分。
