...
describe("SystemPage", () => {
  beforeEach(async () => {
    ...
    mocks.check.mockResolvedValue({
      ...
      target_channel: "main",
      docker_image: "ghcr.io/bjxg/aigw-server",
      docker_tag: "latest",
  ...
  test("keeps long update details contained inside the user-opened dialog", async () => {
    mocks.check.mockResolvedValue({
      ...
      target_channel: "dev",
      docker_image:
        "ghcr.io/bjxg/aigw-server-with-a-very-long-image-name-that-should-not-overflow-the-dialog",
  ...
  test("shows only a short release-notes preview until expanded", async () => {
    mocks.check.mockResolvedValue({
      ...
      docker_image: "ghcr.io/bjxg/aigw-server",
      docker_tag: "latest",
      release_url: "https://github.com/bjxg/aigw-server/releases/tag/v1.2.3",
...
- Change 7`,
    ...
    expect(within(dialog).getByRole("link", { name: /view full release notes/i })).toHaveAttribute(
      "href",
      "https://github.com/bjxg/aigw-server/releases/tag/v1.2.3",
    );
  });
  ...
  test("shows concrete docker versions without release notes when already up to date", async () => {
    mocks.check.mockResolvedValue({
      ...
      latest_commit_url:
        "https://github.com/bjxg/aigw-server/commit/de96948c21de3f0a47a8e1e08cb1b859c73069ba",
      target_channel: "main",
      docker_image: "ghcr.io/bjxg/aigw-server",
      docker_tag: "latest",
  ...
  test("switches to an update console while updating and hides release notes", async () => {
        ...
        logs: [
          {
            ...
            stream: "stdout",
            message: "docker compose pull aigw-server",
          },
          {
            timestamp: "2026-04-20T07:30:02Z",
            stream: "stdout",
            message: "Pulling aigw-server ... done",
          },
        ],
      })
      .mockResolvedValueOnce({
        ...
        logs: [
          ...
          },
          {
            ...
            stream: "stderr",
            message: "Container aigw-server Started",
          },
        ],
      });
    ...
    expect(within(dialog).getByText(/docker compose pull aigw-server/i)).toBeInTheDocument();
    expect(within(dialog).getByText(/Container aigw-server Started/i)).toBeInTheDocument();
