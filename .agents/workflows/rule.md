...
- **仓库**: `bjxg/aigw-panel`
- **本地路径**: `/Users/chengcy/dev/sources/go/aigw-panel`
...
```bash
gh run list --repo bjxg/aigw-panel --branch dev -L 1 --json databaseId,status,conclusion,displayTitle
```

3. 如果状态为 `in_progress`，再等 60 秒后重新检查

```bash
sleep 60 && gh run list --repo bjxg/aigw-panel --branch dev -L 1 --json databaseId,status,conclusion,displayTitle
```

4. 如果状态为 `failure`，查看失败日志

```bash
gh run view <RUN_ID> --repo bjxg/aigw-panel --log-failed 2>&1 | tail -40
```
